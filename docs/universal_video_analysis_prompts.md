...
## Prompt 6: 🎙️ PHÂN TÍCH CẤU TRÚC ÂM THANH NARRATOR ↔ DIALOGUE (Audio Mode Structure)

> **Mục đích:** Thu thập dữ liệu cho `visual_unit_breakdown` (AI Director Voice-over template)
> **Input:** Upload 1-2 video hoàn chỉnh (3-8 phút). Phân tích TOÀN BỘ video — không chỉ trích đoạn. Video nên là dạng **voice-over / narrator-driven** (documentary, educational, story-driven content).

> [!NOTE]
> Prompt này tập trung vào cách video **phối hợp giữa lời kể (narrator) và lời thoại (dialogue)** theo từng shot. Đây là dữ liệu cần thiết để xây dựng AI Director prompt cho các video sử dụng giọng kể thay vì quay thật.

```
Bạn là chuyên gia phân tích audio-visual chuyên nghiên cứu cấu trúc âm thanh trong video voice-over. Tôi cung cấp video từ kênh/video "[TÊN KÊNH / VIDEO]." Hãy giải phẫu CHI TIẾT cách video phối hợp giữa Narrator (người kể chuyện) và Character Dialogue (lời thoại nhân vật) theo từng shot.

Lưu ý: Hãy phân tích DỰA TRÊN THỰC TẾ. Không phải video nào cũng có dialogue — nếu video chỉ có narrator thuần, hãy ghi rõ và vẫn phân tích nhịp điệu narrator.

## 1. PHÂN LOẠI AUDIO MODE TỪNG SHOT (Shot-by-Shot Audio Classification)

Xem toàn bộ video và liệt kê TỪNG shot (mỗi lần hình ảnh thay đổi = 1 shot) vào bảng sau:

| # | Timestamp | Duration | Audio Mode | Narrator? | Dialogue? | Dialogue Content | Nhận xét |
|---|-----------|----------|------------|-----------|-----------|------------------|----------|
| 1 | 00:00-00:05 | 5s | ? | Có/Không | Có/Không | "(nội dung thoại nếu có)" | ? |
| 2 | 00:05-00:09 | 4s | ? | ? | ? | ? | ? |
| ... | ... | ... | ... | ... | ... | ... | ... |

Phân loại Audio Mode theo 2 cấp:
- **narrator_only**: Chỉ có giọng narrator, KHÔNG có dialogue/reaction nào
- **dialogue_dominant**: Nhân vật nói/lên tiếng — có thể có narrator dẫn trước hoặc sau, hoặc narrator ducking volume

## 2. THỐNG KÊ PHÂN BỐ AUDIO MODE

Tổng hợp từ bảng trên:

| Audio Mode | Số shots | % tổng | Tổng thời lượng | % thời lượng |
|------------|----------|--------|-----------------|-------------|
| narrator_only | ? | ? | ? | ? |
| dialogue_dominant | ? | ? | ? | ? |

## 3. PHÂN LOẠI ĐỐI THOẠI (Dialogue Taxonomy)

Nếu video có dialogue, phân loại TỪNG lần dialogue theo bảng:

| # | Timestamp | Speaker | Dialogue Type | Nội dung | Trigger (tại sao lại có dialogue ở đây?) |
|---|-----------|---------|---------------|----------|------------------------------------------|
| 1 | 00:15 | [Moses] | ? | "..." | ? |
| 2 | 00:22 | [CROWD] | ? | "..." | ? |
| ... | ... | ... | ... | ... | ... |

Phân loại Dialogue Type:
- **reaction**: 1-3 từ, phản ứng cảm xúc ("Whoa!", "Ouch!", "Mmm!")
- **soft_line**: 4-8 từ, bình luận ngắn khi narrator vẫn dẫn ("That's amazing!", "Me, King Saul!")
- **full_dialogue**: Hội thoại giữa 2+ nhân vật, narrator im lặng
- **crowd**: Đám đông hô vang, hát, cổ vũ
- **quote**: Trích dẫn nổi tiếng hoặc lời nói mang tính biểu tượng
- **ambient_voice**: Tiếng xì xào, thì thầm, tạo bầu không khí

Phân loại Trigger (lý do dialogue xuất hiện):
- **CHARACTER_INTRO**: Nhân vật mới được giới thiệu
- **EMOTIONAL_PEAK**: Điểm cảm xúc cao trào
- **CONFLICT**: Xung đột giữa các bên
- **ACTION_DESCRIPTION**: Mô tả hành động của nhân vật
- **EXPLANATION_BREAK**: Phá vỡ đơn điệu sau nhiều shot narrator liên tục
- **SYMBOLIC_MOMENT**: Lời nói mang tính biểu tượng/kinh điển
- **ENUMERATION**: Liệt kê (mỗi mục có reaction)
- **CROWD_SCENE**: Nhiều người tham gia

## 4. PHÂN TÍCH NHỊP ĐỘ NARRATOR ↔ DIALOGUE (Pacing Pattern)

### 4a. Mô hình xen kẽ:
- Sau bao nhiêu shot narrator_only thì thường xuất hiện dialogue? (VD: cứ 2-3 shot narrator → 1 shot dialogue)
- Chuỗi narrator liên tục dài nhất là bao nhiêu shot?
- Chuỗi dialogue liên tục dài nhất là bao nhiêu shot/giây?
- Có pattern lặp lại không? Mô tả pattern.

### 4b. Mật độ dialogue theo giai đoạn:
Chia video thành 4 phần (Opening / Middle / Climax / Resolution) và tính:

| Giai đoạn | Timestamp | narrator_only % | dialogue_dominant % | Nhận xét |
|-----------|-----------|-----------------|---------------------|----------|
| Opening (20% đầu) | ? | ? | ? | ? |
| Middle (20-60%) | ? | ? | ? | ? |
| Climax (60-90%) | ? | ? | ? | ? |
...
### 4c. Transition rules (narrator ↔ dialogue):
- Narrator → Dialogue: Narrator nói gì trước khi dialogue bắt đầu? ("...and then he said:", "...but suddenly:") Liệt kê các cue phrases.
- Dialogue → Narrator: Narrator quay lại ngay lập tức hay có khoảng im lặng?
- Trong dialogue_dominant: Narrator có giảm volume (ducking) khi nhân vật nói không? Giảm bao nhiêu %?
- Dialogue có bao giờ kéo dài hơn 15 giây liên tục không?

## 5. VISUAL-AUDIO SYNC (Đồng bộ hình ảnh ↔ âm thanh)

### 5a. Quy tắc cỡ cảnh theo audio mode:
- Khi audio_mode = narrator_only → cỡ cảnh thường là gì? (Wide? Medium? EWS?)
- Khi audio_mode = dialogue_dominant → cỡ cảnh thường là gì? (Close-up? MCU?)

### 5b. Thay đổi cỡ cảnh khi audio mode thay đổi:
- Khi chuyển từ narrator → dialogue: camera có zoom vào không? Chuyển đổi ra sao?
- Khi chuyển từ dialogue → narrator: camera có zoom ra không?
- Có pattern nhất quán không?

### 5c. Character visibility:
- Nhân vật có xuất hiện trên hình khi narrator đang kể không? Hay chỉ thấy bối cảnh/đạo cụ?
- Nhân vật có lip-sync khi nói dialogue không?
- Nhân vật có phản ứng (biểu cảm, cử chỉ) khi narrator kể về họ không?

## 6. SFX & MUSIC LAYER (Lớp âm thanh bổ sung)

### 6a. Sound Effects:
- Liệt kê TẤT CẢ các SFX xuất hiện trong video (theo timestamp)
- Phân loại: {cartoon_SFX | foley | ambient | musical_sting | transition_whoosh | UI_sound}
- SFX có sync với visual action không?
- SFX loại nào hay dùng nhất?

### 6b. Background Music:
- BGM chạy liên tục hay chỉ ở đoạn nhất định?
- BGM volume thay đổi khi dialogue xuất hiện không?
- Có musical climax/swell align với story climax không?

## 7. SAMPLE RECONSTRUCTION (Tái tạo cấu trúc)
Viết một đoạn script MẪU (200-300 từ) về một chủ đề KHÁC nhưng dùng CHÍNH XÁC cùng cấu trúc audio mode (narrator ↔ dialogue rhythm) đã phân tích. Dùng Marked Script Format:

Narrator text here (narrator_only)

[Character] Dialogue here (dialogue_dominant)

Narrator continues... (dialogue_dominant, as it has a character reaction below)
[Character] Reaction!

[SFX] Sound effect description
[CROWD] Crowd sound

Trả lời bằng TIẾNG VIỆT (trừ sample script dùng Marked Script Format).
```
...
| 6 | **Final Synthesis** | Kết quả Prompt 1-5 (+ Prompt 7 nếu có) | 10-15 phút | **Master Reference Sheet** |
| 7 | Audio Mode Structure ⭐ | 1-2 video hoàn chỉnh (3-8 phút) | 20-30 phút | Narrator↔Dialogue rhythm, audio distribution, dialogue taxonomy |

> [!TIP]
> **Prompt 6** chỉ cần dùng nếu video thuộc dạng **voice-over / narrator-driven**. Nếu video là talking-head, vlog, hoặc screen recording thì bỏ qua Prompt 6.

> [!CAUTION]
> Prompt 7 (Final Synthesis) là prompt QUAN TRỌNG NHẤT — nó tổng hợp tất cả kết quả thành format có thể dùng trực tiếp cho template. Đừng bỏ qua bước này!
