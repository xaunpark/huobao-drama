...
const initTaskStates = async () => {
  ...
  for (const sb of localStoryboards.value) {
    ...
    try {
      ...
      const completedVidHd = r2vVideos.find((v: any) => v.status === 'completed' && v.is_upscaled)
      ...
      const failedWithBase = r2vVideos.find((v: any) => (v.status === 'failed' || v.status === 'error') && (v.video_url || v.local_path))

      if (completedVidHd) videoState = 'hd'
      else if (upscalingVid) videoState = 'upscaling'
      else if (completedVid) videoState = 'done'
      else if (failedWithBase) videoState = 'done' // Has a base video even if upscale failed previously
      // R2V Studio should only consider videos generated by the 'direct_r2v' mode.
      // This prevents legacy (I2V) videos from prematurely marking shots as 'Done' here.
    } catch {
      // Ignore
    }
  ...
  try {
    ...
    await runConcurrently(localStoryboards.value, maxConcurrentThreads.value, async (sb) => {
      ...
      try {
        ...
        const target = vidRes.items?.find((v: any) => (v.status === 'completed' && !v.is_upscaled) || (v.status === 'failed' && (v.video_url || v.local_path)))
