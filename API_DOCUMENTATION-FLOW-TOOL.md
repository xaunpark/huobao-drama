...
| `quality` | string | ❌ | `fast` (mặc định) hoặc `quality` |
...
Kích hoạt tính năng Upscale 1080p cho một video đã hoàn thành.

#### Behavior
- Chỉ áp dụng cho Job đã `COMPLETED` hoặc Job Upscale bị `FAILED` (để Retry).
- Cập nhật trạng thái Job hiện tại thành `QUEUED` và Mode thành `UPSCALE`.
- **KHÔNG** tạo Job ID mới.
- Kết quả video upscale sẽ ghi đè lên file video cũ (nếu có).
...
```json
{
  ...
  "status": "QUEUED",
  "mode": "UPSCALE",
  "message": "Job re-queued for 1080p upscale"
}
```
